    ...
    <div>
        ...
        <div class="bg-white shadow overflow-hidden sm:rounded-lg">
            <div class="flex justify-between px-4 py-5 sm:px-6 items-center">
                <h3 class="flex text-lg leading-6 font-medium text-cyan-600">
                ...
                <div v-if="veterinary.user.id === auth">
                    <button @click="openModal"
                            ...
                            :class="veterinary.user.id === auth && ! veterinary.location ? 'animate-bounce' : ''"
                            title="Registrar Ubicación de la Veterinaria">
            ...
            <modal v-if="isAuthenticated && veterinary.user.id === auth"
                   modal-id="veterinary-location"
                   max-width="sm:max-w-5xl">
                <template #title>Ubicación de la Veterinaria</template>
                <template #content>
                    <form @submit.prevent>
                            ...
                            <div class="w-full md:mx-2">
                                <div class="w-full">
                                    <div class="form-group">
                                        ...
                                        <div class="mt-1">
                                            <input type="text"
                                                   v-model="veterinaryLocationForm.address"
                                                   class="lp-input"
                                                   id="address">
                                        </div>
                        ...
                        <div class="w-full md:flex md:-mx-2 mt-4">
                            ...
                            <div class="w-full md:w-1/3 md:mx-2 mt-3 md:mt-0">
                                <div class="form-group">
                                    ...
                                    <div class="mt-1">
                                        <vue-multiselect v-model="veterinaryLocationForm.state"
                                                         :options="states"
                                                         id="state_id"
                                                         label="name"
                                                         track-by="id"
                            ...
                            <div class="w-full md:w-1/3 md:mx-2 mt-3 md:mt-0">
                                <div class="form-group">
                                    ...
                                    <div class="mt-1">
                                        <vue-multiselect v-model="veterinaryLocationForm.city"
                                                         :options="cities"
                                                         id="city"
                                                         :searchable="true"
                                                         :close-on-select="true"
                            ...
                            <div class="w-full md:w-1/3 md:mx-2 mt-3 md:mt-0">
                                <div class="form-group">
                                    ...
                                    <div class="mt-1">
                                        <vue-multiselect v-model="veterinaryLocationForm.neighborhood"
                        ...
                        <div class="w-full md:flex md:-mx-2 mt-4">
                            ...
                            <div class="w-full md:w-1/3 md:mx-2 mt-3 md:mt-0">
                                <div class="form-group">
                                    ...
                                    <div class="mt-1">
                                        <input type="text"
                                               v-model="veterinaryLocationForm.zipCode"
                                               class="lp-input"
                                               id="zip_code">
                                    </div>
...
export default {
    name: "VeterinaryLocation",
    inject: ['veterinary'],
    emits: ['veterinaries.location'],
    data() {
        return {
            endpoint: `/veterinaries/${this.veterinary.slug}/location`,

            location: this.veterinary.location,
            states: [],
            cities: [],
            neighborhoods: [],

            veterinaryLocationForm: {
                address: '',
                neighborhood: '',
                city: '',
                state: {
    ...
    methods: {
        store() {
            axios.post(this.endpoint, {
                'address': this.veterinaryLocationForm.address,
                'neighborhood': this.veterinaryLocationForm.neighborhood,
                'city': this.veterinaryLocationForm.city,
                'state_id': this.veterinaryLocationForm.state ? this.veterinaryLocationForm.state.id : null,
                'zip_code': this.veterinaryLocationForm.zipCode,
            }).then(response => {
                this.closeModal()
                this.location = response.data.data
                window.Event.$emit('veterinaries.location', this.location)
        ...
        update() {
            axios.put(this.endpoint, {
                ...
                'neighborhood': this.veterinaryLocationForm.neighborhood,
                'city': this.veterinaryLocationForm.city,
                'state_id': this.veterinaryLocationForm.state.id,
                'zip_code': this.veterinaryLocationForm.zipCode,
            }).then(response => {
                this.closeModal()
                this.location = response.data.data
        ...
        getCities(selectedState) {
            let state = this.isObject(selectedState) ? selectedState : this.veterinaryLocationForm.state
            axios.get(`/states/${state.name}/cities`)
                .then(response => { this.cities = response.data })
                .catch(error => { console.log(error) })
        },
        getNeighborhoods(selectedCity) {
            let city = selectedCity ? selectedCity : this.veterinaryLocationForm.city
        ...
        openModal() {
            ...
            this.errors = []
            this.modal.id = 'veterinary-location'

            if (this.actionType === 'put') {
                this.veterinaryLocationForm.address = this.location.address
                this.veterinaryLocationForm.neighborhood = this.location.neighborhood
                this.veterinaryLocationForm.city = this.location.city
                this.veterinaryLocationForm.state = this.location.state
                this.veterinaryLocationForm.zipCode = this.location.zipCode

                this.getCities(this.veterinaryLocationForm.state.name)
                this.getNeighborhoods(this.veterinaryLocationForm.city)
            }
